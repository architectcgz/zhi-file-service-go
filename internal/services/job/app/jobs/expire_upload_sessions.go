...
func (j ExpireUploadSessionsJob) ExecuteWithResult(ctx context.Context) (Result, error) {
	processed, err := j.repo.ExpirePendingSessions(ctx, j.clock.Now().Add(-j.expireAfter), j.batchSize)
